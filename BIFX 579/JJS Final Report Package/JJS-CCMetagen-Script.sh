...
# Downloads the reference database.
# -P specifies download location, followed by a link to the online file
...
# Unzips the downloaded file so it can be accessed.
# Just specifies files location
# &> will generate a special error file for this specific command if something goes wrong
unzip /BIODATA/jjs14_capstone/ccmetagen/ncbi_nt_kma.zip &> errorUnzip.txt

# Maps sequences to reference database.
# -ipe indicates paired sequence locations are to follow
# -o specifies output location
# -t_db directs to the database location
...
# Produces the final CCMetagen report.
# -i indicates the mapped input data
# -o indicates the output location
# &> will generate a special error file for this specific command if something goes wrong.
...
# Redo of the final two steps using the downsampled data.
